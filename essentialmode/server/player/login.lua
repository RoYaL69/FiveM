...
function LoadUser(identifier, source, new)
	...
	local result = MySQL:getResults(executed_query, {'permission_level', 'money', 'dirty_money', 'identifier', 'group', 'nom', 'prenom', 'phone_number', 'isFirstConnection'}, "identifier")
	local moneyIcon = "$"
	local group = groups[result[1].group]
	Users[source] = Player(source, result[1].permission_level, result[1].money, result[1].dirty_money, result[1].identifier , group, result[1].nom, result[1].prenom, result[1].phone_number, result[1].isFirstConnection )

	TriggerEvent('es:playerLoaded', source, Users[source])

	TriggerClientEvent('es:setPlayerDecorator', source, 'rank', Users[source]:getPermissions())
	TriggerClientEvent('es:setMoneyIcon', source, moneyIcon)
...
AddEventHandler("es:getPlayerFromIdentifier", function(identifier, cb)
	local executed_query = MySQL:executeQuery("SELECT * FROM users WHERE identifier = '@name'", {['@name'] = identifier})
	local result = MySQL:getResults(executed_query, {'permission_level', 'money', 'identifier', 'group','nom', 'prenom', 'phone_number'}, "identifier")
	...
	SetTimeout(60000, function()
		TriggerEvent("es:getPlayers", function(users)
			...
			if #users == 0 then
				...
				local executed_query = MySQL:executeQuery("SELECT * FROM users WHERE1")
				local result = MySQL:getResults(executed_query, {'permission_level', 'money', 'identifier', 'group'}, "identifier")
			end
		end)
